...
public class TweetsProviderImpl implements TweetsProvider {
	...
	@Nonnull
	private final Map<Long, TweetData> tweets = Collections
			.synchronizedMap(new HashMap<Long, TweetData>());
	...
	@Nonnull
	@Override
	public Status getNewTweet(@Nonnull GamePlayer player, @Nonnull String hash)
	...
	@Nonnull
	@Override
	public Status[] getNewTweets(@Nonnull GamePlayer player, String hash,
	...
	public Status[] getTweets(long[] ids) throws RemoteException {
		...
		for (int i = 0; i < size; i++) {
			...
			result[i] = tweetData != null && tweetData.isReal ? tweetData.tweet
					: null;
		}
	...
	void registerFakeTweet(@Nonnull Status tweet, GamePlayer player,
			@Nonnull String sourceHash) {
		final TweetData checkTweet = tweets.get(tweet.getId());
		if (checkTweet != null) {
			if (!checkTweet.tweet.equals(tweet))
				throw new IllegalArgumentException("Invalid tweet: " + tweet);
			if (checkTweet.isReal)
				throw new IllegalArgumentException("This tweet is not fake! "
						+ tweet);
		} else {
	...
	int[] registerTweet(@Nonnull Status tweet,
			@Nonnull GamePlayer sourcePlayer, @Nonnull String sourceHash) {
		int tweetScore = 1;
		int playerScore = 1;
		synchronized (tweets) {
			...
			if (tweetData == null) {
				registerNewFakeTweet(tweet, sourceHash);
				tweetScore += NEW_FAKE_TWEET_BONUS; // first register of a fake
													// tweet, good for the
													// source
				playerScore += FIRST_REGISTER_BONUS; // first register of a
														// tweet
			} else {
				if (!tweet.equals(tweetData.tweet))
					throw new IllegalArgumentException(
							"There is another tweet with the same id!");
	...
	private void registerNewFakeTweet(@Nonnull Status tweet,
			@Nonnull String sourceHash) {
		synchronized (tweets) {
			final String hashCheck = tweet.generateCheck(sourceHash);
			if (!tweet.getCheck().equals(hashCheck))
				throw new IllegalArgumentException("invalid hash check");
	...
	private Status generateTweet(@Nonnull GamePlayer player,
			@Nonnull String hash) {
		synchronized (tweets) {
			long id = random.nextLong();
			while (tweets.containsKey(id))
				id = random.nextLong();
			final Status status = new Status(id, fortuneWheel.next(),
					player.getId(), hash);
